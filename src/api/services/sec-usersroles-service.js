...
// SERVICIO DE USUARIOS Y ROLES: CRUD Y CONSULTAS AUXILIARES
...
async function RolesCRUD(req) {
  try {
    ...
    if (procedure === 'get' && type === 'all') {
      ...
      const Rolnuevo = await RoleSchema.create(nuevoRol);
      result = Rolnuevo.toObject();
    }
    // --- ELIMINAR ROL ---
...
async function UsersCRUD(req) {
    try {
        ...
        if (procedure === 'post') {
            ...
            // Para controlar lectura, descomentar si es necesario:
